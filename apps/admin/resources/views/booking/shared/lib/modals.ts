...
export const showNotConfirmedReasonDialog = (cancelReasons: CancelReason[]): Promise<ShowNotConfirmedReasonDialogResponse> => new Promise((resolve): void => {
  ...
  const $selectElement = $('<select name="data[reason]" id="form_data_reason" class="form-select form-control" required></select>')
    .change((e) => {
      ...
      const hasDescription = $element.find('option:selected').data('has-description') as boolean
      if (!hasDescription) {
  ...
  $selectElement.append('<option data-has-description="false" value=""></option>')
  cancelReasons.forEach((reason) => {
    $selectElement.append(`<option data-has-description="${reason.has_description}" value="${reason.id}">${reason.name}</option>`)
  })
