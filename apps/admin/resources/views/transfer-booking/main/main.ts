...
$(() => {
  ...
  const $deleteBookingsButton = $('<a />', {
    ...
    if (isConfirmed) {
      toggleLoading()
      await axios.delete('/transfer-booking/bulk', { data: { ids: selectedBookings } })
      location.reload()
    }
  })
  ...
  $('.btn-request-download').on('click', async (e: any) => {
    ...
    const bookingId = $(e.currentTarget).parent().parent().parent()
      .data('id')
    const { data: requests } = await axios.get<BookingRequest[]>(`/transfer-booking/${bookingId}/request/list`)
  ...
  $('.btn-request-send').on('click', async (e: any) => {
    ...
    const bookingId = $(e.currentTarget).parent().parent().parent()
      .data('id')
    const { data: availableActions } = await axios.get<BookingAvailableActionsResponse>(`/transfer-booking/${bookingId}/actions/available`)
    ...
    if (availableActions.isRequestable && requestText) {
      const popoverContentActions: PopoverItem = {
        ...
        callback: async () => {
          await axios.post(`/transfer-booking/${bookingId}/request`)
          location.reload()
        },
      }
