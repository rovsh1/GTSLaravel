...
const props = withDefaults(defineProps<{
  waitingOrders: PaymentBooking[]
  distributedOrders: PaymentBooking[]
...
const emit = defineEmits<{
  (event: 'orders', value: PaymentBookingPayload[]): void
}>()
...
const setDistributedOrders = () => {
  ...
  props.distributedOrders.forEach((distributedOrder) => {
    ...
    const distributedAmount = props.paymentInfo
      ?.landings?.find((landing) => landing.bookingId === distributedOrder.id)?.sum || 0
    distributedOrders.push({ ...source, distributedAmount })
  })
  return distributedOrders
}

const localeWaitingOrders = ref<PaymentBooking[]>([])
...
const getPaymentOrdersPayload = (): PaymentBookingPayload[] => localeDistributedOrders.value.map((order) => ({
  id: order.id,
...
const getIndexToDelete = (array: PaymentBooking[], removedItem: PaymentBooking) => {
...
const getClientPrice = (order: PaymentBooking) => order.supplierPenalty?.value || order.supplierPrice.value
          ...
          <tbody>
            <template v-if="localeWaitingOrders.length">
              ...
              >
                ...
                <td>{{ formatPrice(getClientPrice(waitingOrder), waitingOrder.supplierPrice.currency.value) }}</td>
                <td>
                  {{ formatPrice((getClientPrice(waitingOrder)
                    - waitingOrder.remainingAmount.value), waitingOrder.supplierPrice.currency.value) }}
                </td>
                <td>
          ...
          <table class="table table-bordered mb-0 w-100 table-sm">
            ...
            <tbody>
              <template v-if="localeDistributedOrders.length">
                ...
                >
                  <td>{{ order.id }}</td>
                  <td>{{ formatPrice(getClientPrice(order), order.supplierPrice.currency.value) }}</td>
                  <td>
                    {{ formatPrice((getClientPrice(order) - order.remainingAmount.value),
                                   order.supplierPrice.currency.value) }}
                  </td>
                  <td>
