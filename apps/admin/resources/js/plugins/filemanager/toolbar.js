...
class Toolbar {
	...
	get el() {
		...
		el.find('button.btn-folder-add').click(function () {
			InputBox({
				placeholder: 'Название папки',
				handler: function (name) { self.#manager.createFolder(name); }
			});
		});
	...
	delete() {
		if (confirm('Выделенные файлы будут безвозвратно удалены! Продолжить?'))
