...
export default class Reader {
  ...
  sendFormData(formData, url) {
    ...
    xhr.upload.addEventListener('error', () => { self.trigger('error') }, false)

    xhr.onreadystatechange = function () {
      if (this.readyState !== 4) return

      if (this.status === 200) {
        if (uploaded) self.trigger('complete', JSON.parse(this.responseText))
        else {
          self.trigger('error', {
            code: this.status,
  ...
  unbind(event, callback) {
    if (undefined === this.#eventHandlers) return this
