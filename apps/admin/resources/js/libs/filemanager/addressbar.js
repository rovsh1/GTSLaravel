class AddressBar {
  ...
  get el() {
    ...
    el.find('button.btn-refresh').click(() => { self.#manager.refresh() })
    el.find('button.btn-up').click(() => { self.#manager.goUp() })
    el.find('input')
      .keyup(function (e) {
        if (e.keyCode === 27) this.value = ''
        else if (e.keyCode === 13) self.#manager.search(this.value)
        else self.#manager.search(this.value)
      })
  ...
  setFilter(type) {
    ...
    if (type) f.find(`button.btn-${type}`).addClass('selected')
  }
}
