...
export default class Thumb {
  ...
  get el() {
    ...
    el.find('div.btn-remove').click(() => { self.delete() })
  ...
  data(data) {
    ...
    } else html += `<img src="${getMimeImage(data.mime_type)}" />`
  ...
  upload(file) {
    ...
    reader
      ...
      .bind('complete', (data) => {
        ...
        } else html += `<img src="${getMimeImage(file.type)}" />`
        html += '</a>'
  ...
  getById(fileId) {
    const { el } = this
    el.addClass('loading')
    Http.getJSON(`/file/${fileId}/data`, (data) => {
  ...
  remove() {
    this.el.remove()
    if (this.isNew() && this.get('filename')) Http.post(`/tmp/${this.get('filename')}/unlink`)
  }
  ...
  delete() {
    if (this.isNew()) {
      ...
      if (!confirm('Файл будет безвозвратно удален. Продолжить?')) return
